...
import type { ClientDocument, Client, ClientLink } from "@/types"

const CLIENTS_COLLECTION = "clients"


function documentToClient(docId: string, data: any): Client {
...
export async function getClientByCode(code: string): Promise<Client | null> {
	...
	try {
		...
		return documentToClient(doc.id, doc.data())
	} catch (error: any) {
...
export async function getClientById(clientId: string): Promise<Client | null> {
	try {
		...
		return documentToClient(docSnap.id, docSnap.data())
	} catch (error) {
